...
const validateFullname = () => {
  ...
  if (!fullname) {
    ...
    errors.value.fullname = 'Họ và tên phải có từ 2-50 ký tự';
...
.profile-container {
  ...
  margin-top: -20px;
  padding: 20px 10px;
  /* Đảm bảo không bị ảnh hưởng bởi admin layout */
  position: relative;
  z-index: 1;
}
...
/* Profile Sidebar - Đổi tên để tránh xung đột với admin sidebar */
.profile-sidebar {
  width: 300px;
  background: white !important;
  ...
  /* Tránh xung đột với admin layout */
  position: relative;
  z-index: 2;
}
...
/* Modal Styles - Đảm bảo hiển thị trên admin layout */
.modal-overlay {
  position: fixed;
  top: 0;
  left: 0;
